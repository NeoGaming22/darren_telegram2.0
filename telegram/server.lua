				...
				exports.ghmattimysql:execute("INSERT INTO telegrams (sender, recipient, recipientid, message) VALUES (@sender, @recipient, @recipientid, @message)",  paramaters, function()
					for k, v in pairs(players) do
						...
						if receiver == firstname .. " " .. lastname then 
							TriggerClientEvent("vorp:TipBottom", _source, "You've received a telegram.", 3000)
						end
					end
				end)
				TriggerClientEvent("vorp:TipBottom", _source, "Your telegram has been posted!", 3000)
			else 
				TriggerClientEvent("vorp:TipBottom", _source, "Unable to process Telegram. Invalid first or lastname.", 3000)
			end
		end)
	end
end)
